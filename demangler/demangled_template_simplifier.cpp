...
namespace
{
	...
	static bool StartsWith(std::string_view s, const char* prefix)
	{
		size_t n = strlen(prefix);
		return s.compare(0, n, prefix) == 0;
	}

	static std::string_view StripLeadingTypeKeyword(std::string_view s)
	{
		s = TrimSpaces(s);
	...
	static bool StringEquals(const string& s, std::string_view value)
	{
		return s.size() == value.size() &&
			(value.empty() || memcmp(s.data(), value.data(), value.size()) == 0);
	}

	static bool IsUnqualifiedNonPointerType(const DemangledTypeNode& type)
	{
		return !type.IsConst() && !type.IsVolatile() && type.GetPointerSuffixBits() == 0;
	}

	static size_t ResolvedIntegerWidth(size_t width, DemangledTypeNode::WidthKind widthKind)
	{
		switch (widthKind)
		{
	...
	static bool GetUnqualifiedIntegerTypeInfo(const DemangledTypeNode& type, size_t& width,
	...
	static bool GetUnqualifiedWideCharTypeInfo(const DemangledTypeNode& type, size_t& width,
		std::string_view& altName)
	{
		return IsUnqualifiedNonPointerType(type) && type.GetWideCharTypeInfo(width, altName);
	}

	static string CanonicalTypeNoSpaces(const DemangledTypeNode& type)
	{
		string s = type.GetString();
		return RemoveSpaces(StripLeadingTypeKeyword(std::string_view(s.data(), s.size())));
	}

	static string CanonicalTypeNoSpaces(std::string_view type)
	{
		return RemoveSpaces(StripLeadingTypeKeyword(type));
	}

	static bool SameType(const DemangledTypeNode& a, const DemangledTypeNode& b)
	{
	...
	static bool SameType(const DemangledTypeNode& a, std::string_view b)
	{
	...
	static bool IsSimpleNamedType(const DemangledTypeNode& type, std::string_view name, bool cnst = false)
	{
	...
	static bool IsQualifiedName(const DemangledTypeNode& type, std::initializer_list<std::string_view> expected)
	{
	...
	static const char* CharFamilyAliasName(const DemangledTypeNode& element, const char* charAlias,
	...
	static bool HasOnlyOmittedPointerSuffixes(const DemangledTypeNode& type)
	{
		return (type.GetPointerSuffixBits() & ~(1u << Ptr64Suffix)) == 0;
	}

	static bool IsConstTypeNamed(const DemangledTypeNode& type, std::string_view name)
	{
	...
	static bool IsInlineStdNamespace(const DemangledNamePart& component)
	{
	...
	static bool IsAbslInlineNamespace(const DemangledNamePart& component)
	{
		if (component.HasTemplateArguments())
			return false;
		const string& name = component.GetBase();
	...
	static bool IsStdScope(const DemangledQualifiedName& name, size_t* templateIndex)
	{
	...
	static bool IsStdScope(const DemangledTypeNode& type, size_t* templateIndex)
	{
		const DemangledQualifiedName& name = type.GetName();
		return IsStdScope(name, templateIndex);
	}

	static bool IsAbslScope(const DemangledQualifiedName& name, size_t* templateIndex)
	{
	...
	static bool IsAbslScope(const DemangledTypeNode& type, size_t* templateIndex)
	{
		const DemangledQualifiedName& name = type.GetName();
		return IsAbslScope(name, templateIndex);
	}

	static bool IsStdTemplate(const DemangledTypeNode& type, std::string_view tmpl, const ParamList** args = nullptr)
	{
	...
	static bool IsAbslTemplate(const DemangledTypeNode& type, std::string_view tmpl, const ParamList** args = nullptr)
	{
		size_t templateIndex = 0;
	...
	static bool IsAbslNestedTemplate(const DemangledTypeNode& type, std::string_view scope, std::string_view tmpl,
	...
	static bool IsAbslNestedName(const DemangledTypeNode& type, std::string_view scope, std::string_view base)
	{
		size_t scopeIndex = 0;
		if (!IsAbslScope(type, &scopeIndex))
			return false;
	...
	static bool IsStdPmrTemplate(const DemangledTypeNode& type, std::string_view tmpl, const ParamList** args = nullptr)
	{
		const DemangledQualifiedName& name = type.GetName();
	...
	static bool IsStdOneArgTemplate(const DemangledTypeNode& type, std::string_view tmpl, const DemangledTypeNode& arg)
	{
		const ParamList* args = nullptr;
	...
	static bool IsStdOneArgTemplate(const DemangledTypeNode& type, std::string_view tmpl, std::string_view arg)
	{
		const ParamList* args = nullptr;
		if (!IsStdTemplate(type, tmpl, &args) || args->size() != 1 || !(*args)[0].type)
			return false;
	...
	static bool IsStdPmrOneArgTemplate(const DemangledTypeNode& type, std::string_view tmpl, const DemangledTypeNode& arg)
	{
		const ParamList* args = nullptr;
	...
	static bool IsPmrPolymorphicAllocator(const DemangledTypeNode& type, const DemangledTypeNode& valueType)
	{
		return IsStdPmrOneArgTemplate(type, "polymorphic_allocator", valueType);
	}

	static bool IsConstTypeOf(const DemangledTypeNode& type, const DemangledTypeNode& base)
	{
		if (type.IsConst() && !type.IsVolatile())
		{
			DemangledTypeNode unqualified = type;
	...
	static bool IsStdPairOfConstKeyValue(const DemangledTypeNode& type, const DemangledTypeNode& key,
	...
	static bool IsStdPairOfKeyValue(const DemangledTypeNode& type, const DemangledTypeNode& key,
		const DemangledTypeNode& value)
	{
		const ParamList* pairArgs = nullptr;
		if (!IsStdTemplate(type, "pair", &pairArgs) || pairArgs->size() != 2 || !(*pairArgs)[0].type ||
	...
	static bool IsDefaultPairAllocator(const DemangledTypeNode& type, const DemangledTypeNode& key,
		const DemangledTypeNode& value)
	{
	...
	static bool IsDefaultPairAllocatorWithMutableKey(const DemangledTypeNode& type, const DemangledTypeNode& key,
		const DemangledTypeNode& value)
	{
		const ParamList* allocArgs = nullptr;
		if (!IsStdTemplate(type, "allocator", &allocArgs) || allocArgs->size() != 1 || !(*allocArgs)[0].type)
	...
	static bool IsPmrPairAllocator(const DemangledTypeNode& type, const DemangledTypeNode& key,
		const DemangledTypeNode& value)
	{
		const ParamList* allocArgs = nullptr;
	...
	static bool IsAbslOneArgTemplate(const DemangledTypeNode& type, std::string_view tmpl, const DemangledTypeNode& arg)
	{
		const ParamList* args = nullptr;
	...
	static std::string_view AbiBaseName(std::string_view name);

	static bool IsStdStringLike(const DemangledTypeNode& type)
	{
		const DemangledQualifiedName& name = type.GetName();
		size_t templateIndex = 0;
	...
	static bool IsAbslStringViewLike(const DemangledTypeNode& type)
	{
		size_t templateIndex = 0;
		if (!IsAbslScope(type, &templateIndex))
			return false;
	...
	static bool IsAbslDefaultStringLikeKey(const DemangledTypeNode& key)
	{
		return IsStdStringLike(key) || IsAbslStringViewLike(key);
	}

	static bool IsAbslNestedOneArgTemplate(const DemangledTypeNode& type, std::string_view scope, std::string_view tmpl,
	...
	static bool IsAbslHashDefault(const DemangledTypeNode& type, const DemangledTypeNode& key)
	{
	...
	static bool IsAbslEqDefault(const DemangledTypeNode& type, const DemangledTypeNode& key)
	{
	...
	static bool IsBoostScope(const DemangledQualifiedName& name, size_t* templateIndex)
	{
	...
	static bool IsBoostScope(const DemangledTypeNode& type, size_t* templateIndex)
	{
		const DemangledQualifiedName& name = type.GetName();
		return IsBoostScope(name, templateIndex);
	}

	static bool IsBoostTemplate(const DemangledTypeNode& type, std::string_view tmpl, const ParamList** args = nullptr)
	{
		size_t templateIndex = 0;
	...
	static bool IsBoostOneArgTemplate(const DemangledTypeNode& type, std::string_view tmpl, const DemangledTypeNode& arg)
	{
		const ParamList* args = nullptr;
	...
	static bool GetSimpleLiteralName(const DemangledTypeNode& arg, std::string_view& literal)
	{
	...
	static bool IsIntegerSuffix(char c)
	{
		return c == 'u' || c == 'U' || c == 'l' || c == 'L';
	}

	static bool MatchesNonTypeValue(std::string_view raw, std::string_view value)
	{
		raw = TrimSpaces(raw);
	...
	static bool IsNonTypeValue(const DemangledTypeNode& arg, std::string_view value)
	{
		std::string_view literal;
		return GetSimpleLiteralName(arg, literal) && MatchesNonTypeValue(literal, value);
	}

	static bool CanonicalIntegerLiteral(std::string_view raw, string& out)
	{
		raw = TrimSpaces(raw);
		bool hadCast = false;
		size_t castClose = raw.rfind(')');
	...
	static bool CanonicalizeNonTypeIntegerLiteral(DemangledTypeNode& type)
	{
		std::string_view literal;
	...
	static bool IsVoidType(const DemangledTypeNode& arg)
	{
		return SameType(arg, "void");
	}

	static bool IsPtrdiffType(const DemangledTypeNode& arg)
	{
	...
	static bool IsPointerTo(const DemangledTypeNode& type, const DemangledTypeNode& pointee)
	{
	...
	static bool IsReferenceTo(const DemangledTypeNode& type, const DemangledTypeNode& referenced)
	{
		const DemangledTypeNode* child = nullptr;
		BNReferenceType referenceType = PointerReferenceType;
		return !type.IsConst() && !type.IsVolatile() && HasOnlyOmittedPointerSuffixes(type) &&
	...
	static bool IsConstPointerTo(const DemangledTypeNode& type, std::string_view pointee)
	{
		const DemangledTypeNode* child = nullptr;
		BNReferenceType referenceType = PointerReferenceType;
		return !type.IsConst() && !type.IsVolatile() && HasOnlyOmittedPointerSuffixes(type) &&
	...
	static bool IsStringConstIterator(const DemangledTypeNode& type, bool wide, bool pmr)
	{
	...
	static const char* RegexIteratorAliasName(const DemangledTypeNode& iterator, const char* charPointerAlias,
	...
	static bool IsSubMatchOfIterator(const DemangledTypeNode& type, const DemangledTypeNode& iterator, bool pmr = false)
	{
	...
	static bool IsDefaultSubMatchAllocator(const DemangledTypeNode& type, const DemangledTypeNode& iterator)
	{
		const ParamList* allocArgs = nullptr;
		if (!IsStdTemplate(type, "allocator", &allocArgs) || allocArgs->size() != 1 || !(*allocArgs)[0].type)
			return false;
	...
	static bool IsPmrSubMatchAllocator(const DemangledTypeNode& type, const DemangledTypeNode& iterator)
	{
		const ParamList* allocArgs = nullptr;
		if (!IsStdPmrTemplate(type, "polymorphic_allocator", &allocArgs) || allocArgs->size() != 1 ||
			!(*allocArgs)[0].type)
	...
	static const char* WideStringName(std::string_view name)
	{
	...
	static const char* AtomicAliasForName(std::string_view t)
	{
	...
	static const char* AtomicAliasForPrimitive(const DemangledTypeNode& type)
	{
	...
	static const char* AtomicAliasForNamedType(const DemangledTypeNode& type)
	{
		if (!IsUnqualifiedNonPointerType(type) || type.GetClass() != NamedTypeReferenceClass)
			return nullptr;
	...
	static const char* AtomicAliasName(const DemangledTypeNode& type)
	{
	...
	static void ResizeTemplateArgs(DemangledNamePart& c, size_t n)
	{
	...
	static std::string_view AbiTagSuffix(std::string_view name)
	{
		size_t suffixStart = name.size();
		while (suffixStart > 0 && name[suffixStart - 1] == ']')
		{
	...
	static void ReplaceWithAlias(DemangledNamePart& c, std::string_view alias)
	{
	...
	static bool ApplyStringRule(DemangledNamePart& c)
	{
	...
	static bool ApplyIostreamRule(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
	...
	static bool ApplyContainerRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (args.empty())
	...
	static bool ApplyDiscoveredStdRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (args.empty())
	...
	static bool ApplyChronoRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
	...
	static bool ApplyGenericStdDefaultRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
	...
	static bool ApplyStdFunctionTemplateRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
	...
	static bool ApplyStdTemplateRules(DemangledNamePart& c)
	{
	...
	static bool ApplyAbslTemplateRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (args.empty())
	...
	static bool ApplyBoostContainerStringRule(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
	...
	static bool ApplyBoostContainerTemplateRules(DemangledNamePart& c)
	{
	...
	static bool ApplyBoostUnorderedTemplateRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (args.empty())
	...
	static bool ApplyPmrAliasToStdTemplate(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (args.empty())
	...
	static bool ApplyConcreteStdAliasRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (args.empty())
	...
	static bool ApplyPmrConcreteAliasRules(DemangledNamePart& c)
	{
		ParamList& args = c.GetMutableTemplateArguments();
		const std::string_view name = AbiBaseName(c.GetBase());
		if (name == "basic_string" && args.size() == 1 && args[0].type)
	...
	static void SimplifyTemplateArguments(DemangledNamePart& c, bool& changed)
	{
	...
	static bool ApplyNamespaceRules(DemangledQualifiedName& name)
	{
		bool changed = false;
	...
	static bool ApplyStdRules(DemangledQualifiedName& name)
	{
		bool changed = false;
	...
	static bool ApplyAbslRules(DemangledQualifiedName& name)
	{
		bool changed = false;
		for (size_t i = 1; i < name.size(); i++)
		{
	...
	static bool ApplyBoostRules(DemangledQualifiedName& name)
	{
		bool changed = false;
		for (size_t i = 1; i < name.size(); i++)
		{
	...
	static AbiTaggedName SplitAbiTagSuffix(std::string_view name)
	{
		size_t suffixStart = name.size();
		while (suffixStart > 0 && name[suffixStart - 1] == ']')
		{
	...
	static string AddAbiTags(const string& base, const string& tags)
	{
	...
	static bool ConstructorNameMatches(const string& previousBase, const string& currentBase)
	{
	...
	static bool RewriteSimplifiedConstructorSegments(DemangledQualifiedName& name)
	{
		bool changed = false;
		for (size_t i = 1; i < name.size(); i++)
		{
	...
	static bool IsAliasName(std::string_view alias, const unordered_set<std::string_view>& aliases)
	{
		return aliases.find(alias) != aliases.end();
	}

	static bool IsTopLevelSimplifiedAlias(const DemangledQualifiedName& name)
	{
	...
	static bool HasTemplateArgs(const DemangledQualifiedName& name)
	{
	...
	static size_t FindTemplateOpen(std::string_view component)
	{
	...
	static vector<std::string_view> SplitTopLevel(std::string_view s, char delimiter)
	{
	...
	static DemangledTypeNode ParseCompatibilityType(std::string_view s);

	static ParamList ParseCompatibilityArgs(std::string_view s)
	{
		ParamList args;
	...
	static DemangledQualifiedName ParseCompatibilityName(std::string_view s)
	{
	...
	static StringList RenderSegments(const DemangledQualifiedName& name)
	{
